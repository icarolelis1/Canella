...
void Canella::RenderSystem::VulkanBackend::Descriptorpool::buildGlobalDescriptorPool(Device &device)
{
	...
	if ((vkCreateDescriptorPool(device.getLogicalDevice(), &pool_info, device.getAllocator(), &vk_global_descriptorpool) != VK_SUCCESS))
...
void Canella::RenderSystem::VulkanBackend::Descriptorpool::buildBindlessDescriptorPool(Device &device)
{
	...
	VkResult result = vkCreateDescriptorPool(device.getLogicalDevice(), &create_info, device.getAllocator(), &vk_bindless_descriptorpool);
...
void Descriptorpool::AllocateDescriptorSet(Device &device, DescriptorSetLayout &layout, std::vector<VkDescriptorSet> &sets)
{
